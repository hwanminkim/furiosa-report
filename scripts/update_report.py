...
- **4~6** — '{subject}'가 다른 주체와 함께한 협력·MOU·연동·도입·채택. **제목 주어가 상대(기관/기업)여도, 그 협력의 한 축이 '{subject}'이고 '{subject}'의 제품·기술이 구체적으로 쓰이면 이 구간.** (예: "ETRI, {subject}와 NPU 지원센터 협력" / "○○대, {subject} 칩 도입")
- **0~3** — 여러 회사 중 하나로 **나열**되거나 곁다리 언급, **정부/대규모 투자·펀드·메가프로젝트 기사에서 수혜기업 중 하나로 언급**, 경쟁사 비교, 단순 시황·주가·관련주·특징주, 제품이 부품으로 한 줄 언급 → 리포트 제외. (예: "3대 메가프로젝트 3000조 투자", "삼성·SK 1000조 투자" 류 산업 전반 기사에 '{subject}'가 끼어 언급되면 0~3)

## 판단 원칙
- "회사가 문법상 주체(actor)냐 객체냐"를 기계적으로 따지지 말 것. 기준은 **BD 담당자에게 유용한 시그널인가**이다.
- **핵심 구분**: '{subject}'와의 **구체적 협력·제품 채택**(상대가 누구든) > 산업 전반 거대 투자·정책 기사에 '{subject}'가 **이름만 끼인 것**. 후자는 규모가 커 보여도 0~3이다.
...
def score_relevance(subject: str, aliases: list[str], articles: list[dict], client: "OpenAI | None", body_gate: bool = False) -> list[dict]:
    ...
    for i, a in enumerate(gated):
        ...
        if score >= RELEVANCE_THRESHOLD:
            kept.append(a)
    # 진단용: 점수 높은 순으로 게이트 통과분 점수 출력
    for a in sorted(gated, key=lambda x: x.get("relevance", 0), reverse=True):
        print(f"  [score {a.get('relevance', 0)}] {a.get('title', '')[:75]}")
    return kept
